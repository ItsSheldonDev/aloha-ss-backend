import {
  ...
  export class DocumentsController {
    ...
    @Get(':id/count')
    @ApiOperation({ summary: 'Récupérer le nombre de téléchargements d\'un document (accès public)' })
    @ApiParam({ name: 'id', description: 'ID du document' })
    @ApiResponse({ status: 200, description: 'Nombre de téléchargements récupéré avec succès' })
    @ApiResponse({ status: 404, description: 'Document non trouvé' })
    async getDownloadCount(@Param('id') id: string) {
      const count = await this.documentsService.getDownloadCount(id);
      return { downloads: count };
    }
  
    @Post(':id/count')
    @ApiOperation({ summary: 'Incrémenter le compteur de téléchargements (accès public)' })
    @ApiParam({ name: 'id', description: 'ID du document' })
    @ApiResponse({ status: 200, description: 'Compteur incrémenté avec succès' })
    @ApiResponse({ status: 404, description: 'Document non trouvé' })
    async incrementDownloadCount(@Param('id') id: string) {
      const count = await this.documentsService.incrementDownloadCount(id);
      return { success: true, downloads: count };
    }
